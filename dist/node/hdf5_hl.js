...
class HasAttrs {
    ...
    create_attribute(name, data, shape, dtype) {
        ...
        if (metadata.vlen) {
            Module.create_vlen_str_attribute(this.file_id, this.path, name, prepared_data, final_shape, metadata.type, metadata.size, metadata.signed, metadata.vlen);
        }
        else {
            let data_ptr = Module._malloc(prepared_data.byteLength);
            try {
                Module.HEAPU8.set(prepared_data, data_ptr);
                Module.create_attribute(this.file_id, this.path, name, BigInt(data_ptr), final_shape, metadata.type, metadata.size, metadata.signed, metadata.vlen);
            }
            finally {
                Module._free(data_ptr);
            }
...
export class Group extends HasAttrs {
    ...
    *items() {
    ...
    create_dataset(args) {
        ...
        if (metadata.vlen) {
            Module.create_vlen_str_dataset(this.file_id, this.path + "/" + name, prepared_data, final_shape, final_maxshape, final_chunks, metadata.type, metadata.size, metadata.signed, metadata.vlen, track_order ?? false);
        }
        else {
            let data_ptr = Module._malloc(prepared_data.byteLength);
            try {
                Module.HEAPU8.set(prepared_data, data_ptr);
                Module.create_dataset(this.file_id, this.path + "/" + name, BigInt(data_ptr), final_shape, final_maxshape, final_chunks, metadata.type, metadata.size, metadata.signed, metadata.vlen, compression_id, compression_opts_out, track_order ?? false);
            }
            finally {
                Module._free(data_ptr);
            }
